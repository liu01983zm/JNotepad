...
/**
*该文章转载自网络大本营：http://www.xrss.cn/Dev/JAVA/20081918449.Html
*
*/
// 自定义日历类
...
is 29 days
*/
  private static final int[] iLunarMonthDaysTable = { 0x4ae0, 0xa570, 
0x5268, 0xd260, 0xd950, 0x6aa8, 0x56a0, 0x9ad0, 0x4ae8,
      0x4ae0, // 1910
...
0x00, 0x20, // 1910
  ...
  private static final char[] iSolarLunarOffsetTable = { 49, 38, 28, 46, 
34, 24, 43, 32, 21, 40, // 1910
...
|| (iMonth == 8) || (iMonth == 10) || (iMonth == 12))
      return 31;
    else if ((iMonth == 4) || (iMonth == 6) || (iMonth == 9) || (iMonth == 
11))
      return 30;
    else if (iMonth == 2) {
  ...
  static int iGetLMonthDays(int iYear, int iMonth) {
    ...
    if ((iMonth > 12) && (iMonth - 12 != iLeapMonth) || (iMonth < 0)) {
      System.out.println("Wrong month, ' , i think you are want a -1, go to death!");
      return -1;
    }
  ...
  static String sCalendarSolarToLundar(int iYear, int iMonth, int iDay) {
    ...
    if (iOffsetDays < iSolarLunarOffsetTable[iYear - 1901]) {
      ...
      for (iLMonth = 12; iOffsetDays > iGetLMonthDays(iLYear, iLMonth); 
iLMonth--) {
        iLDay = iOffsetDays;
...
(iLDay > 9 ? "" + iLDay : "0" + iLDay);
   ...
   // The solar calendar is turned into the lunar calendar
  static LunarDate CalendarSolarToLundar(int iYear, int iMonth, int iDay) {
    int iLDay, iLMonth, iLYear;
    int iOffsetDays = iGetSNewYearOffsetDays(iYear, iMonth, iDay);
    int iLeapMonth = iGetLLeapMonth(iYear);
...
iLMonth--) {
	...
	int month = iLMonth > 9 ? iLMonth :  iLMonth;
    int day=iLDay > 9 ? iLDay : iLDay;
    return  new LunarDate(iLYear,month,day);
	//	"" + iLYear + (iLMonth > 9 ? "" + iLMonth : "0" + iLMonth) + 
   //(iLDay > 9 ? "" + iLDay : "0" + iLDay);
  }
...
(iSDay > 9 ? iSDay + "" : "0" + iSDay);
  ...
  // The lunar calendar is turned into the Solar calendar
  static SolarDate CalendarLundarToSolar(int iYear, int iMonth, int iDay) {
    int iSYear, iSMonth, iSDay;
    int iOffsetDays = iGetLNewYearOffsetDays(iYear, iMonth, iDay) 
		              + iSolarLunarOffsetTable[iYear - 1901];
    int iYearDays = bIsSolarLeapYear(iYear) ? 366 : 365;
    ...
    //return "" + iSYear + (iSMonth > 9 ? iSMonth + "" : "0" + iSMonth) + 
    //  (iSDay > 9 ? iSDay + "" : "0" + iSDay);
    int month= iSMonth > 9 ? iSMonth : iSMonth;
    int day = iSDay > 9 ? iSDay  : iSDay;
    return new SolarDate(iSYear,month,day);
  }
}
